...
exports.downloadInvoicePDF = exports.getInvoiceStats = exports.getInvoicesByUserId = exports.deleteInvoice = exports.updateInvoice = exports.getInvoices = exports.deleteInvoiceByOrderId = exports.updateInvoicePaymentStatus = exports.createInvoice = void 0;
...
const generateInvoiceHtml = (invoice, baseUrl) => {
    const logoUrl = "https://res.cloudinary.com/adaired/image/upload/f_auto,q_auto/v1/Static%20Website%20Images/adaired_logo"; // Replace with actual logo URL
    ...
    const statusColor = statusColorMap[String(invoice.status)] ||
        "background-color: #3b82f6; color: #ffffff;";
    // Generate table rows for order items
    const tableRows = invoice.orderId.products
        .map((item, index) => `
      <tr>
        <td style="padding: 8px 16px; border-bottom: 1px solid #e5e7eb;">${index + 1}</td>
        <td style="padding: 8px 16px; border-bottom: 1px solid #e5e7eb;">
          <h6 style="font-size: 14px; font-weight: 500; margin: 0;">${item.product.name}</h6>
        </td>
        ...
        <td style="padding: 8px 16px; border-bottom: 1px solid #e5e7eb;">${item.quantity}</td>
        <td style="padding: 8px 16px; border-bottom: 1px solid #e5e7eb;">${item.wordCount}</td>
        <td style="padding: 8px 16px; border-bottom: 1px solid #e5e7eb; font-weight: 500;">$${item.totalPrice}</td>
      </tr>
    `)
        .join("");
    return `
          ...
          <div class="grid">
            <div>
              ...
              <div class="text-sm">
                ...
                <p>${(0, formatDate_1.formatDate)(invoice.createdAt)}</p>
              </div>
            </div>
            <div>
              <h6 class="text-sm font-semibold">Bill To</h6>
              <p class="text-sm font-semibold uppercase">${invoice.userId?.name || "N/A"}</p>
              <div class="text-sm">
                <p class="font-semibold">Due Date</p>
                <p>${(0, formatDate_1.formatDate)(invoice.dueDate)}</p>
              </div>
            </div>
            <div class="qr-code">
              <img src="https://api.qrserver.com/v1/create-qr-code/?size=112x112&data=${encodeURIComponent(qrCodeValue)}" alt="QR Code" />
            </div>
          </div>
          <table class="table">
            <thead>
...
// ******************* Create an invoice *******************
// *********************************************************
const createInvoice = async (orderId, paymentMethod) => {
    const order = await orderModel_1.default.findById(orderId).populate("userId");
    if (!order) {
        throw new error_1.CustomError(404, "Order not found.");
    }
    // Check if invoice already exists
    const existingInvoice = await invoice_model_1.default.findOne({ orderId });
    if (existingInvoice) {
        throw new error_1.CustomError(400, "Invoice already exists for this order.");
    }
    ...
    await orderModel_1.default.findByIdAndUpdate(orderId, { invoiceId: invoiceNumber });
    // Send emails asynchronously
    Promise.all([
        (0, mailer_1.sendInvoiceGeneratedEmail)(newInvoice._id.toString()),
        (0, mailer_1.sendAdminNewInvoiceEmail)(newInvoice._id.toString()),
...
// *********************************************************
const updateInvoicePaymentStatus = async (orderId, paymentStatus) => {
    const invoice = await invoice_model_1.default.findOne({ orderId });
    if (invoice) {
        const newStatus = paymentStatus === "Paid"
            ...
            : paymentStatus === "Failed"
                ...
                : paymentStatus === "Refunded"
                    ? "Refunded"
                    : "Unpaid";
        await invoice_model_1.default.findByIdAndUpdate(invoice._id, { status: newStatus });
    }
};
...
// *********************************************************
const deleteInvoiceByOrderId = async (orderId) => {
    await invoice_model_1.default.findOneAndDelete({ orderId });
};
exports.deleteInvoiceByOrderId = deleteInvoiceByOrderId;
// **************************************************************
...
const getInvoices = async (req, res, next) => {
    try {
        ...
        // Check Permission
        const permissionCheck = await (0, authHelper_1.checkPermission)(userId, "invoices", 2);
        if (!permissionCheck) {
            return res.status(403).json({ message: "Permission denied" });
        }
        let invoices;
        if (invoiceNumber) {
            invoices = await invoice_model_1.default.findOne({ invoiceNumber })
            ...
            invoices = await invoice_model_1.default.find()
                .sort({ createdAt: -1 })
                .populate("userId", "-wishlist -orderHistory -refreshToken -createdAt -updatedAt -__v")
                .populate({
                path: "orderId",
        ...
        next(new error_1.CustomError(500, error instanceof Error ? error.message : "An unknown error occurred."));
    }
};
...
const updateInvoice = async (req, res, next) => {
    try {
        ...
        // Check Permission
        const permissionCheck = await (0, authHelper_1.checkPermission)(userId, "invoices", 3);
        if (!permissionCheck) {
            return res.status(403).json({ message: "Permission denied" });
        }
        const updatedInvoice = await invoice_model_1.default.findByIdAndUpdate(invoiceId, updateData, {
            new: true,
        });
        if (!updatedInvoice) {
            return res.status(404).json({ message: "Invoice not found." });
...
const deleteInvoice = async (req, res, next) => {
    try {
        ...
        const { invoiceId } = req.query;
        // Check Permission
        const permissionCheck = await (0, authHelper_1.checkPermission)(userId, "invoices", 4);
        if (!permissionCheck) {
            return res.status(403).json({ message: "Permission denied" });
        }
        const deletedInvoice = await invoice_model_1.default.findByIdAndDelete(invoiceId);
        if (!deletedInvoice) {
            return res.status(404).json({ message: "Invoice not found." });
        }
        // Remove invoiceId from associated order
        await orderModel_1.default.findOneAndUpdate({ invoiceId: deletedInvoice.invoiceNumber }, { invoiceId: null });
        res
            .status(200)
...
const getInvoicesByUserId = async (req, res, next) => {
    try {
        ...
        let invoices;
        if (invoiceNumber) {
            invoices = await invoice_model_1.default.findOne({ userId, invoiceNumber })
                .populate("userId", "-wishlist -orderHistory -refreshToken -createdAt -updatedAt -__v")
                .populate({
                path: "orderId",
                populate: {
            ...
            invoices = await invoice_model_1.default.find({ userId })
                .populate("userId", "-wishlist -orderHistory -refreshToken -createdAt -updatedAt -__v")
                .populate({
                path: "orderId",
                populate: {
...
exports.getInvoicesByUserId = getInvoicesByUserId;
// **************************************************************
...
const getInvoiceStats = async (req, res, next) => {
    try {
        ...
        const newInvoicesChange = prevMonthNewInvoices === 0
            ...
            : ((currentMonthNewInvoices - prevMonthNewInvoices) /
                ...
                100;
        const totalAmountChange = prevMonthTotalAmount === 0
            ? allTimeTotalAmount > 0
                ? 100
                : 0
            : ((allTimeTotalAmount - prevMonthTotalAmount) / prevMonthTotalAmount) *
                100;
        const finalAmountChange = prevMonthFinalAmount === 0
...
const downloadInvoicePDF = async (req, res, next) => {
    ...
    try {
        ...
        if (!invoiceNumber || typeof invoiceNumber !== "string") {
            throw new error_1.CustomError(400, "Invoice number is required.");
        }
        // Fetch invoice data
        const invoice = await invoice_model_1.default.findOne({ invoiceNumber })
            .populate("userId", "-wishlist -orderHistory -refreshToken -createdAt -updatedAt -__v")
            .populate({
            path: "orderId",
            populate: {
            ...
            .lean();
        if (!invoice) {
            throw new error_1.CustomError(404, "Invoice not found.");
        }
        // Launch puppeteer
        ...
        page.on("error", (err) => {
            ...
            throw new error_1.CustomError(500, "Failed to process page content.");
        });
        ...
        const imageResults = await page.evaluate(() => {
            ...
            return Promise.all(images.map((img) => {
                ...
                return new Promise((resolve) => {
                    ...
                    img.onerror = () => {
                        console.warn(`[downloadInvoicePDF] Image failed to load: ${img.src}`);
