...
exports.getCouponStats = exports.deleteCoupon = exports.getCoupons = exports.updateCoupon = exports.calculateCouponDiscount = exports.createCoupon = exports.recordCouponUsage = exports.applyCoupon = exports.calculateDiscount = void 0;
...
const validateDiscountVsMinOrder = (coupon) => {
    if (coupon.minOrderAmount && coupon.discountType === "flat") {
        if (coupon.discountValue > coupon.minOrderAmount) {
            throw new error_1.CustomError(400, `Discount value ($${coupon.discountValue}) cannot exceed the minimum order amount ($${coupon.minOrderAmount}) for a flat discount coupon`);
        }
    }
};
// Helper function to calculate discount
const calculateDiscount = (coupon, cartData) => {
    let discount = 0;
    ...
    if (coupon.couponApplicableOn === "specificProducts") {
        eligibleProducts = cartData.products.filter((p) => coupon.specificProducts.some((sp) => sp.equals(typeof p.product === "object" ? p.product._id : p.product)));
    }
    else if (coupon.couponApplicableOn === "productCategories") {
        eligibleProducts = cartData.products.filter((p) => coupon.productCategories.some((cat) => cat.equals(typeof p.product === "object" && "category" in p.product
            ? p.product.category
            : p.product)));
    }
    if ((coupon.couponApplicableOn === "specificProducts" ||
        ...
        throw new error_1.CustomError(400, `No qualifying products found for coupon "${coupon.code}"`);
    }
    // Special handling for 100% off coupons (percentage only)
    if (coupon.discountType === "percentage" && coupon.discountValue === 100) {
        const qualifyingProducts = eligibleProducts.filter((product) => !coupon.maxWordCount || product.wordCount <= coupon.maxWordCount);
        if (qualifyingProducts.length === 0) {
            throw new error_1.CustomError(400, coupon.maxWordCount
                ? `Coupon "${coupon.code}" requires items with ≤ ${coupon.maxWordCount} words`
                : `No items qualify for coupon "${coupon.code}"`);
        }
        const productToDiscount = qualifyingProducts.reduce((lowest, current) => current.totalPrice < lowest.totalPrice ? current : lowest);
        if (productToDiscount.quantity !== 1) {
            throw new error_1.CustomError(400, `"${coupon.code}" applies to single-item purchases only. Please remove other items and set quantity to 1 to enjoy this discount.`);
        }
    ...
    if (coupon.couponType === "quantityBased") {
        const hasEnoughQuantity = eligibleProducts.some((p) => p.quantity >= (coupon.minQuantity || 1));
        if (!hasEnoughQuantity) {
            throw new error_1.CustomError(400, `Minimum quantity of ${coupon.minQuantity} required for coupon "${coupon.code}"`);
        }
    }
    // Calculate discount based on discountType
    const eligibleTotal = eligibleProducts.reduce((sum, p) => sum + p.totalPrice, 0);
    if (coupon.discountType === "percentage") {
        if (cartData.totalPrice < (coupon.minOrderAmount || 0)) {
            throw new error_1.CustomError(400, "Minimum order amount not met");
        }
        ...
        throw new error_1.CustomError(400, "Invalid discount type");
    }
    appliedTo = eligibleProducts.map((p) => typeof p.product === "object" ? p.product._id : p.product);
    discountedTotal = Math.max(0, cartData.totalPrice - discount);
    return { discount, discountedTotal, appliedTo };
};
exports.calculateDiscount = calculateDiscount;
// Helper to validate and apply coupon
const applyCoupon = async (couponCode, cart, userId) => {
    let discountUSD = 0;
    ...
    if (!coupon) {
        throw new error_1.CustomError(404, "Invalid or expired coupon");
    }
    // Check is coupon is already used to its limit
    if (coupon.usedCount >= coupon.totalUsageLimit) {
        throw new error_1.CustomError(400, `Coupon "${coupon.code}" has reached its usage limit`);
    }
    // Validate discount vs minimum order amount
    validateDiscountVsMinOrder(coupon);
    // Check usage limits
    ...
    if (userUsage && userUsage.usageCount >= coupon.usageLimitPerUser) {
        throw new error_1.CustomError(400, `You've reached the usage limit for coupon "${coupon.code}"`);
    }
    if (coupon.usedCount >= coupon.totalUsageLimit) {
        throw new error_1.CustomError(400, `Coupon "${coupon.code}" has reached its total usage limit`);
    }
    const { discount, discountedTotal, appliedTo: discountedProducts = [], } = (0, exports.calculateDiscount)(coupon, {
    ...
    if (coupon.couponApplicableOn === "specificProducts") {
        if (appliedTo.length === 1) {
            const product = cart.products.find((p) => p.product._id.equals(appliedTo[0]));
            message = `Discount applied to "${product?.product?.name || "your item"}"`;
        }
        ...
        const product = cart.products.find((p) => p.product._id.equals(appliedTo[0]));
        message = `100% discount applied to "${product?.product?.name || "your item"}"${coupon.maxWordCount ? ` (max ${coupon.maxWordCount} words)` : ""}`;
    }
...
const recordCouponUsage = async (couponId, userId) => {
    ...
    if (!coupon) {
        throw new error_1.CustomError(404, "Coupon not found");
    }
    // Update coupon usage
    const userUsage = coupon.userUsage?.find((u) => u.userId.toString() === userId);
    if (userUsage) {
...
const createCoupon = async (req, res, next) => {
    try {
        ...
        // Check Permission
        const permissionCheck = await (0, authHelper_1.checkPermission)(userId, "coupons", 0);
        if (!permissionCheck) {
...
// ******************* Apply Coupon ***********************
// *********************************************************
const calculateCouponDiscount = async (req, res, next) => {
    try {
        ...
        if (!coupon) {
            return next(new error_1.CustomError(404, "Invalid or expired coupon"));
        }
        // Check is coupon is already used to its limit
        if (coupon.usedCount >= coupon.totalUsageLimit) {
            return next(new error_1.CustomError(400, `Coupon "${coupon.code}" has reached its usage limit`));
        }
        // Validate discount vs minimum order amount
        validateDiscountVsMinOrder(coupon);
        const { discount, discountedTotal, appliedTo = [], } = (0, exports.calculateDiscount)(coupon, cartData);
        // Build product discounts map with proportional distribution
        const productDiscounts = {};
        const eligibleProducts = cartData.products.filter((p) => appliedTo.some((id) => id.toString() === p.product._id.toString()));
        const eligibleTotal = eligibleProducts.reduce((sum, p) => sum + p.totalPrice, 0);
        // Always distribute the final (capped) discount proportionally
        const finalDiscount = Math.min(discount, coupon.maxDiscountAmount || Infinity);
        if (eligibleTotal === 0) {
            throw new error_1.CustomError(400, "No eligible products with valid prices");
        }
        eligibleProducts.forEach((product) => {
            const productShare = product.totalPrice / eligibleTotal;
            productDiscounts[product.product._id.toString()] = Number((finalDiscount * productShare).toFixed(2));
        });
        res.status(200).json({
            success: true,
            ...
            couponDetails: {
                ...
                maxDiscountAmount: coupon.maxDiscountAmount !== Infinity
                    ...
                    : null,
                minOrderAmount: coupon.minOrderAmount !== 1 ? coupon.minOrderAmount : null,
                minQuantity: coupon.minQuantity !== 1 ? coupon.minQuantity : null,
                maxWordCount: coupon.maxWordCount,
                specificProducts: coupon.couponApplicableOn === "specificProducts"
                    ? await product_model_1.default.find({
                        _id: { $in: coupon.specificProducts },
                    }).select("name")
                    : [],
                productCategories: coupon.couponApplicableOn === "productCategories"
                    ? await product_category_model_1.default.find({
        ...
        next(new error_1.CustomError(500, error instanceof Error ? error.message : "An error occurred"));
    }
};
...
const updateCoupon = async (req, res, next) => {
    try {
        ...
        if (!id) {
            throw new error_1.CustomError(400, "Coupon ID is required in query parameters");
        }
        // Check Permission
        const permissionCheck = await (0, authHelper_1.checkPermission)(userId, "coupons", 2);
        if (!permissionCheck) {
            throw new error_1.CustomError(403, "Permission denied");
        }
        // Find existing coupon
        const existingCoupon = await coupon_model_1.default.findById(id);
        if (!existingCoupon) {
            throw new error_1.CustomError(404, "Coupon not found");
        }
        // Apply updates
        ...
        const updatedCoupon = await coupon_model_1.default.findByIdAndUpdate(id, updates, {
            new: true,
            runValidators: true,
        });
        res.status(200).json({
...
const getCoupons = async (req, res, next) => {
    try {
        ...
        // Check Permission
        const permissionCheck = await (0, authHelper_1.checkPermission)(userId, "coupons", 1);
        if (!permissionCheck) {
            throw new error_1.CustomError(403, "Permission denied");
        }
        ...
        const coupons = await coupon_model_1.default.find(queryParams)
            .sort({ createdAt: -1 })
            .populate("createdBy", "_id name image email")
            .populate("updatedBy", "_id name image email");
        res.status(200).json({
...
const deleteCoupon = async (req, res, next) => {
    try {
        ...
        const { id } = req.query;
        // Check Permission
        const permissionCheck = await (0, authHelper_1.checkPermission)(userId, "coupons", 3);
        if (!permissionCheck) {
            throw new error_1.CustomError(403, "Permission denied");
        }
...
const getCouponStats = async (req, res, next) => {
    try {
        ...
        const stats = await coupon_model_1.default.aggregate([
            {
                $group: {
                    _id: null,
                    totalCoupons: { $sum: 1 },
        ...
        const popularCoupons = await coupon_model_1.default.find()
            .sort({ usedCount: -1 })
            .limit(5)
