...
exports.getOrdersByUserId = exports.getSalesReport = exports.getOrderStats = exports.deleteOrder = exports.updateOrder = exports.getOrders = exports.stripeWebhook = exports.createOrder = void 0;
...
const getCurrencyFromRegion = async (ip) => {
    try {
        const response = await axios_1.default.get(`https://ipinfo.io/${ip}/json?token=${process.env.IPINFO_TOKEN}`);
        return response.data.country === "IN" ? "inr" : "usd";
    }
    catch (error) {
        if (error instanceof Error) {
            console.error("Error detecting region:", error);
            throw new error_1.CustomError(500, error.message);
        }
        return "usd";
    }
};
...
const createStripeSession = async (cart, orderNumber, currency, exchangeRate, coupon, discountUSD, userId) => {
    const discountLocal = discountUSD * exchangeRate;
    return stripe.checkout.sessions.create({
        payment_method_types: ["card"],
        line_items: cart.products.map((item) => {
            const product = item.product;
            const unitAmountUSD = item.wordCount && item.wordCount > 0
        ...
        ...(discountLocal > 0 && coupon?.discountType !== "PRODUCT_SPECIFIC"
            ...
            : {}),
        mode: "payment",
        success_url: `${globals_1.BASE_DOMAIN}/expert-content-solutions/order/order-confirmation/${orderNumber}`,
        cancel_url: `${globals_1.BASE_DOMAIN}/expert-content-solutions`,
...
const createOrder = async (req, res, next) => {
    try {
        ...
        if (!userId) {
            return next(new error_1.CustomError(401, "User must be logged in to create an order"));
        }
        const cart = await cartModel_1.default.findOne({ userId }).populate("products.product");
        ...
        if (couponCode) {
            const couponResult = await (0, coupon_controller_1.applyCoupon)(couponCode, cart, userId);
        ...
        if (finalPriceUSD === 0) {
            ...
            const session = await createStripeSession(cart, orderNumber, currency, exchangeRate, coupon, discountUSD, userId);
            newOrder = new orderModel_1.default({
                ...orderData,
        ...
        await (0, invoice_controller_1.createInvoice)(newOrder._id.toString(), paymentMethod);
        await cart.updateOne({ products: [], totalPrice: 0, totalQuantity: 0 });
        Promise.all([
            (0, mailer_1.sendOrderConfirmationEmail)(newOrder._id.toString()),
            (0, mailer_1.sendAdminNewOrderEmail)(newOrder._id.toString()),
        ]).catch((err) => console.error("Email sending failed:", err));
        const response = {
            message: "Order created successfully.",
            data: newOrder,
            redirectUrl: finalPriceUSD === 0
                ? `${globals_1.BASE_DOMAIN}/expert-content-solutions/order/order-confirmation/${orderNumber}`
                : undefined,
    ...
    switch (event.type) {
        case "checkout.session.completed":
            ...
            if (session.payment_status === "paid") {
                const updatedOrder = await orderModel_1.default.findOneAndUpdate({ paymentId: session.id }, {
                    paymentStatus: "Paid",
                    paymentDate: Date.now(),
                }, { new: true });
                if (updatedOrder) {
                    // Update invoice payment status
                    await (0, invoice_controller_1.updateInvoicePaymentStatus)(updatedOrder._id.toString(), "Paid");
                    // Record coupon usage after successful payment
                    if (session.metadata.couponId) {
                        await (0, coupon_controller_1.recordCouponUsage)(new mongoose_1.Types.ObjectId(session.metadata.couponId), session.metadata.userId);
                    }
                    // Send emails asynchronously
                    Promise.all([
                        (0, mailer_1.sendPaymentConfirmationEmail)(updatedOrder._id.toString()),
                        (0, mailer_1.sendAdminPaymentReceivedEmail)(updatedOrder._id.toString()),
                    ]).catch((err) => console.error("Email sending failed:", err));
                }
            }
            break;
        case "checkout.session.expired":
            ...
            const updatedOrder = await orderModel_1.default.findOneAndUpdate({ paymentId: expiredSession.id }, { paymentStatus: "Unpaid" }, { new: true });
            if (updatedOrder) {
                await (0, invoice_controller_1.updateInvoicePaymentStatus)(updatedOrder._id.toString(), "Unpaid");
                ...
                if (cart && cart.products.length > 0) {
                    ...
                    await orderModel_1.default.findOneAndUpdate({ paymentId: expiredSession.id }, {
...
const getOrders = async (req, res, next) => {
    try {
        ...
        // Check Permission
        const permissionCheck = await (0, authHelper_1.checkPermission)(userId, "orders", 2);
        if (!permissionCheck) {
            return res.status(403).json({ message: "Permission denied" });
        }
        let orders;
        if (orderNumber) {
            orders = await orderModel_1.default.findOne({ orderNumber: orderNumber })
        ...
        if (error instanceof Error) {
            ...
            next(new error_1.CustomError(500, "An unknown error occurred."));
        }
    }
};
...
const updateOrder = async (req, res, next) => {
    try {
        ...
        // Check Permission
        const permissionCheck = await (0, authHelper_1.checkPermission)(userId, "orders", 3);
        if (!permissionCheck) {
            return res.status(403).json({ message: "Permission denied" });
        }
        if (updateData.paymentStatus) {
            // Update invoice payment status
            await (0, invoice_controller_1.updateInvoicePaymentStatus)(orderId.toString(), updateData.paymentStatus);
        }
        const updatedOrder = await orderModel_1.default.findByIdAndUpdate(orderId, updateData, {
            new: true,
        });
        if (!updatedOrder) {
            return res.status(404).json({ message: "Order not found." });
...
const deleteOrder = async (req, res, next) => {
    try {
        ...
        const { orderId } = req.query;
        // Check Permission
        const permissionCheck = await (0, authHelper_1.checkPermission)(userId, "orders", 4);
        if (!permissionCheck) {
            return res.status(403).json({ message: "Permission denied" });
        }
        const deletedOrder = await orderModel_1.default.findByIdAndDelete(orderId);
        if (!deletedOrder) {
            return res.status(404).json({ message: "Order not found." });
        }
        // Delete associated invoice
        await (0, invoice_controller_1.deleteInvoiceByOrderId)(orderId.toString());
        res
            .status(200)
...
const getOrderStats = async (req, res, next) => {
    try {
        ...
        const [stats] = await orderModel_1.default.aggregate([
            {
                $facet: {
                    currentMonthOrders: [
        ...
        const newOrdersChange = prevMonthNewOrders === 0
...
const getOrdersByUserId = async (req, res, next) => {
    try {
        ...
        let orders;
        if (orderNumber) {
            orders = await orderModel_1.default.findOne({ userId, orderNumber: orderNumber })
                .populate({
                path: "products.product",
                populate: {
                    path: "category",
            ...
            orders = await orderModel_1.default.find({ userId })
                .populate({
                path: "products.product",
                populate: {
                    path: "category",
