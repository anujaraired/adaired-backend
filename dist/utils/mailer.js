...
exports.sendAdminNewInvoiceEmail = exports.sendInvoiceGeneratedEmail = exports.sendAdminPaymentReceivedEmail = exports.sendAdminNewOrderEmail = exports.sendPaymentConfirmationEmail = exports.sendOrderConfirmationEmail = exports.sendEmail = void 0;
...
// Base email template
const emailTemplate = (subject, number, type, greeting, date, totalAmount, paymentStatus, linkUrl, linkText, closing) => `
<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
...
const sendOrderConfirmationEmail = async (orderId) => {
    try {
        ...
        const linkUrl = paymentStatus === "Unpaid"
            ? order.paymentUrl
        ...
        const html = emailTemplate(subject, number, type, greeting, date, totalAmount, paymentStatus, linkUrl, linkText, closing);
        await (0, exports.sendEmail)(user.email, `${subject} - Order #${number}`, html);
    }
    catch (error) {
...
const sendPaymentConfirmationEmail = async (orderId) => {
    try {
        const order = await orderModel_1.default.findById(orderId).populate("userId");
        if (!order || !order.userId)
            throw new Error("Order or user not found");
...
// Send new order notification to all admins
const sendAdminNewOrderEmail = async (orderId) => {
    try {
        const order = await orderModel_1.default.findById(orderId).populate("userId");
        if (!order || !order.userId)
            throw new Error("Order or user not found");
        const adminEmails = admins.filter((email) => !!email);
        ...
        const closing = `Best Regards,<br>Team Adaired`;
        const html = emailTemplate(subject, number, type, greeting, date, totalAmount, paymentStatus, linkUrl, linkText, closing);
        await Promise.all(adminEmails.map((email) => (0, exports.sendEmail)(email, `${subject} - Order #${number}`, html)));
    }
    catch (error) {
...
const sendAdminPaymentReceivedEmail = async (orderId) => {
    try {
        const order = await orderModel_1.default.findById(orderId).populate("userId");
        if (!order || !order.userId)
            throw new Error("Order or user not found");
...
exports.sendAdminPaymentReceivedEmail = sendAdminPaymentReceivedEmail;
// Send invoice generated email to user
const sendInvoiceGeneratedEmail = async (invoiceId) => {
    try {
        ...
        const closing = `Best Regards,<br>Team Adaired`;
        const html = emailTemplate(subject, number, type, greeting, date, totalAmount, paymentStatus, linkUrl, linkText, closing);
        await (0, exports.sendEmail)(user.email, `${subject} - Invoice #${number}`, html);
    }
    catch (error) {
...
const sendAdminNewInvoiceEmail = async (invoiceId) => {
    try {
        const invoice = await invoice_model_1.default.findById(invoiceId)
            .populate("userId")
            .populate("orderId");
        ...
        const closing = `Best Regards,<br>Team Adaired`;
        const html = emailTemplate(subject, number, type, greeting, date, totalAmount, paymentStatus, linkUrl, linkText, closing);
        await Promise.all(adminEmails.map((email) => (0, exports.sendEmail)(email, `${subject} - Invoice #${number}`, html)));
    }
    catch (error) {
        console.error("Failed to send admin new invoice email:", error);
    }
};
exports.sendAdminNewInvoiceEmail = sendAdminNewInvoiceEmail;
